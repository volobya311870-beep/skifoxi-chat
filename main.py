...
class MeshChatApp(App):
    ...
    def build(self):
        ...
        # Сборка тактических слоев интерфейса (Идеальный порядок!)
        ...
        # Главный terminal вывода сообщений
        self.chat_logs = TextInput(text="Система SkiFoxi active, Ser. Вирусный меш-протокол взведен.\n", readonly=True, multiline=True)
        self.chat_layout.add_widget(self.chat_logs)
        
        # Поле ввода текста (Для теста формата отправки: "ID_ЦЕЛИ:Сообщение")
        ...
        # ТАКТИЧЕСКИЙ РЕВИЗОР: Каждые 5 секунд очищает эфир от пропавших узлов
            ...
            try:
                ...
                if data:
                    ...
                    if raw_msg.startswith(self.NETWORK_SIGNATURE):
                        ...
                        peer_ip = addr[0]  # УЛЬТИМАТИВНОЕ РЕШЕНИЕ: Вырезаем чистую строку IP из кортежа адреса!
                        ...
                        if clean_msg.startswith("INFO:"):
                            ...
                            # Безопасная потоковая передача в главный GUI-поток
    ...
    def transfer_specific_chunk(self, msg_id, chunk_num, peer_ip):
        try:
            ...
            # Точечный пакет данных отправляется строго на чистый IP соседа peer_ip без индексов и портов
            packet = f"{self.NETWORK_SIGNATURE}DATA:{msg_id}:{chunk_num}:{info['total']}:{info['sender']}:{info['target']}|{info['text']}"
            self.mesh_socket.sendto(packet.encode('utf-8'), (str(peer_ip), 50001))
        except:
            pass
    ...
    def update_radar(self, peer_id, peer_name, peer_ip, current_dbm):
        ...
        try:
            ...
            if peer_id in self.active_peers:
                ...
                self._refresh_button_ui(peer_id, button_text, btn_color)
            else:
                self.active_peers[peer_id] = {
                    'ip': peer_ip, 'name': peer_name, 'last_seen': time.time(), 'signal': current_dbm
                }
                self._create_button_ui(peer_id, button_text, btn_color)
